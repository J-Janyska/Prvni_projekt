...
occ_col_width = max(10, max(word_length_occurences.values())) + 2
print(line, f"LEN|{'OCCURENCES':^{occ_col_width}}|NR.", line, sep="\n")

for length in sorted(word_length_occurences.keys()):
    occurence = word_length_occurences[length]
    print(f"{length:>3}|{'*' * occurence:<{occ_col_width}}|{occurence}")
